...
if (demo) { byId('demo-banner').hidden = false; document.title = 'Demo — Review Packet'; document.querySelector('link[rel="canonical"]')?.setAttribute('href', 'https://review-pdf-packet.sociobot.in/demo'); for (const [selector, value] of [['meta[property="og:title"]', 'Demo — Review Packet'], ['meta[property="og:description"]', 'Inspect a complete sample PDF review packet and reset it at any time.'], ['meta[name="twitter:title"]', 'Demo — Review Packet'], ['meta[name="twitter:description"]', 'Inspect a complete sample PDF review packet and reset it at any time.']] as const) document.querySelector(selector)?.setAttribute('content', value); byId('route-announcer').textContent = 'Demo route loaded.'; if (savedDemoDraft?.title) restoreDemoFiles(); else seedDemo(); requestAnimationFrame(() => byId('hero-title').focus()); byId('reset-demo').addEventListener('click', () => { clearDemoStorage(); seedDemo(); }); byId('start-real').addEventListener('click', () => clearDemoStorage()); } else { renderPdf(); renderContext(); renderAttachments(); renderLinks(); updatePreview(); }
