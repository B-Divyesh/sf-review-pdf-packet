...
Demo text uses `demo:review-packet:*` local-storage keys. Normal drafts use `review-packet:*` keys. Demo files are created in memory and never enter either namespace. Use **Reset demo** to restore the sample. Use **Open empty builder** to remove all demo keys and return to an empty builder.
